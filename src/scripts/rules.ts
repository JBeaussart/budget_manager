...
const inputPattern = document.getElementById('rule-pattern') as HTMLInputElement | null
const inputCategory = document.getElementById('rule-category') as HTMLInputElement | null
const inputEnabled = document.getElementById('rule-enabled') as HTMLInputElement | null
const rulesBody = document.getElementById('rules-body') as HTMLElement | null
const rulesExport = document.getElementById('rules-export') as HTMLButtonElement | null
const rulesImport = document.getElementById('rules-import') as HTMLButtonElement | null

const monthInput = document.getElementById('rc-month') as HTMLInputElement | null
const applyBtn = document.getElementById('rules-apply') as HTMLButtonElement | null
const commitBtn = document.getElementById('rules-commit') as HTMLButtonElement | null
const allCheckbox = document.getElementById('rc-all') as HTMLInputElement | null
...
const previewBody = document.getElementById('rules-preview-body') as HTMLElement | null
const previewSummary = document.getElementById('rules-summary') as HTMLElement | null
const exportChangesBtn = document.getElementById('rules-export-changes') as HTMLButtonElement | null
  ...
  for (const r of rules) {
    ...
    tr.innerHTML = `
      <td class="px-3 py-2 text-slate-700">
        <input type="checkbox" data-action="toggle" data-id="${r.id}" ${r.enabled ? 'checked' : ''} class="h-4 w-4 rounded border-slate-300 text-emerald-600" />
      </td>
      <td class="px-3 py-2 text-slate-700">${r.pattern}</td>
      <td class="px-3 py-2 text-slate-700">${r.category}</td>
      <td class="px-3 py-2 text-right">
        <div class="inline-flex gap-2">
          <button data-action="up" data-id="${r.id}" class="rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50">↑</button>
          <button data-action="down" data-id="${r.id}" class="rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50">↓</button>
          <button data-action="delete" data-id="${r.id}" class="rounded-lg border border-slate-300 bg-white px-3 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50">Supprimer</button>
        </div>
      </td>
    `
...
function applyRulesLocally(rows: Tx[], rules: Rule[]) {
  ...
  const changes: Array<{ id: string; occurred_at: string; description: string; counterparty: string; oldCategory: string; newCategory: string }> = []
  ...
  for (const row of rows) {
    ...
    for (const rule of active) {
      ...
      if (!pat) continue
      const fieldsToCheck: Array<'description' | 'counterparty'> = rule.field ? [rule.field] : ['description', 'counterparty']
      const matched = fieldsToCheck.some((f) => norm((row as any)[f]).includes(pat))
      if (matched) {
...
applyBtn?.addEventListener('click', async () => {
  try {
    ...
    const rows = await fetchScope(monthInput?.value || null, !!allCheckbox?.checked)
    const changes = applyRulesLocally(rows, loadRules())
    renderPreview(changes)
    setFeedback(changes.length ? 'Prévisualisation prête.' : 'Aucun changement proposé.', changes.length ? 'success' : 'info')
    exportChangesBtn!.onclick = () => exportChangesCSV(changes)
    if (commitBtn) commitBtn.onclick = () => doCommit(changes)
  } catch (err) {
    console.error(err)
    setFeedback(err instanceof Error ? err.message : 'Erreur lors de la prévisualisation.', 'error')
  }
})
...
async function doCommit(changes: ReturnType<typeof applyRulesLocally>) {
  try {
    ...
    setFeedback(`Catégories mises à jour: ${payload.updated}/${updates.length}.`, 'success')
    preview?.classList.add('hidden')
  } catch (err) {
    console.error(err)
    setFeedback(err instanceof Error ? err.message : 'Erreur lors de l\'application.', 'error')
  }
}
