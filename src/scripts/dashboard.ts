...
function updateCardsAndPie() {
  ...
  if (state.allMonths) {
    if (cardIncomeAvg)
      cardIncomeAvg.textContent = `Moyenne mensuelle: ${fmt(inc / 12)}`;
    if (cardExpensesAvg)
      cardExpensesAvg.textContent = `Moyenne mensuelle: ${fmt(exp / 12)}`;
  } else {
    if (cardIncomeAvg) cardIncomeAvg.textContent = "";
    if (cardExpensesAvg) cardExpensesAvg.textContent = "";
  }
  if (pieMonthLabel)
    pieMonthLabel.textContent = state.allMonths
      ? `Année ${state.year}`
      : `${monthsFull[state.month - 1]} ${state.year}`;
