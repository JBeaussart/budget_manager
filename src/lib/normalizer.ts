...
export function normalizeRow(row: any, map: Record<string, string>): NormalizedTx {
  ...
  if (!Number.isFinite(amount)) {
    ...
    const debitKey = Object.keys(row).find((k) => k.toLowerCase().includes('débit') || k.toLowerCase().includes('debit'))
    const creditKey = Object.keys(row).find((k) => k.toLowerCase().includes('crédit') || k.toLowerCase().includes('credit'))
  ...
  if (map.type) {
    ...
    if (/(debit|déb|sortie|retrait|prél|prelev|paiement|cb|carte)/.test(typeVal)) {
  ...
  const counterparty = map.counterparty ? String(row[map.counterparty] ?? '').trim() || undefined : undefined
  const category = map['category'] ? String(row[map['category']] ?? '').trim() || undefined : undefined
